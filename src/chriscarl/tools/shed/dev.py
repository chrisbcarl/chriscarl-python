...
def audit_tdd(dirpath=REPO_DIRPATH, module_name=chriscarl.__name__, tests_dirname='tests'):
...
def create_modules_and_tests(root_module, modules, descriptions, author='', email='', tests_dirpath='', cwd=os.getcwd(), force=False, tool=False, no_test=False):
    # type: (str, List[str], dict, str, str, str, str, bool, bool, bool) -> Tuple[List[str], List[str]]
    pwd = os.getcwd()
    os.chdir(cwd)
    try:
        created_filepaths_dirpaths: Tuple[List[str], List[str]] = ([], [])
        ...
        for m, module in enumerate(modules):
            ...
            if not any([os.path.isdir(root_src_directory), os.path.isdir(root_src_directory)]):
                ...
                created_filepaths_dirpaths[1].append(abspath(root_src_directory))
                root_directory = 'src'
            ...
            for t, token in enumerate(tokens[:-1]):
                ...
                created_filepaths_dirpaths[1].append(abspath(current_directory))
            ...
            if doit:
                ...
                created_filepaths_dirpaths[0].append(abspath(module_filepath))
            ...
            if no_test:
                ...
                created_filepaths_dirpaths[1].append(abspath(test_root_directory))
                ...
                for t, token in enumerate(tokens):
                    ...
                    if doit:
                        ...
                        created_filepaths_dirpaths[0].append(abspath(test_relpath))
        ...
        return created_filepaths_dirpaths
    finally:
        os.chdir(pwd)
